...
class DeviationHistoryAdmin(ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        for business_deviation in deviations:
            for deviation in business_deviation['data']:
                if ((deviation['value'] >= 0 and deviation['pct_deviation_sign'] == 0)
                        or (deviation['value'] < 0 and deviation['pct_deviation_sign'] == 1)):
                    deviation['is_grow'] = True
                else:
                    deviation['is_grow'] = False

                if deviation['value'] >= 0:
                    deviation['color'] = True
                else:
                    deviation['color'] = False
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).none()  #super y parentna, deviations table db um chka vor none veradarcni
